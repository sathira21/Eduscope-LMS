...
- `QuizAttempt.student`: Index for retrieving a specific student's history.
- `QuizAttempt.quiz`: Index for generating analytics per quiz.

## Data Life Cycle
- **Archive Policy**: Attempts older than 2 years are moved to long-term storage.
- **Audit Logs**: All sensitive operations are logged in the `ActivityLog` collection.
